	...
	var LockScreen = {
		...
		formEvents: function( $form ) {
			...
			$form.on('submit', function (e) {
				...
				var data = {
					...
					pwdDogrulama: $('#Dogrulama').val()
				};
				var pwd = $('.profile-info').attr('data-lock-password');
				if (data.pwdSifre == pwd) {

					if (data.pwdYeni != "" && data.pwdDogrulama != "") {
						if (data.pwdYeni.length >= 6) {

							if (data.pwdYeni != data.pwdDogrulama) {
								$("#desc").text("Þifreler eþleþmiyor!");
							}
							else {
								if (data.pwdYeni == data.pwdSifre) {
									$("#desc").text("Eski þifreyle yeni þifre ayný olamaz.");
								}
								else {
									$.post("/Home/ChangePassword", { formData: data.pwdYeni },
										function (result) {

											$("#desc").text(result.message);
											setTimeout(function () { window.location.href = result.url; }, 3000);
							...
							$("#desc").text("Þifreniz en az 6 karakterli olmalýdýr!");
                        }
						...
						if (data.pwdYeni != "") {
							...
							if (data.pwdDogrulama == "") {
								$("#desc").text("Þifreyi doðrulayýnýz!");
							}
						}
						else {
							$("#desc").text("Yeni þifre giriniz!");
                        }
					...
					if (data.pwdSifre == "") {
						$("#desc").text("Þifre boþ girilemez!");
					}
					else {
						$("#desc").text("Eski þifre yanlýþ!");
                    }
				}
		...
		buildTemplate: function( userinfo ) {
			return [
				...
				'<input id="Sifre" name="pwdSifre" type="password"  class="form-control" placeholder="Eski Þifre"/> <br>',
				'<input id="YeniSifre" name="pwdYeni" type="password" class="form-control " placeholder="Yeni Þifre" /> <br>',
				'<input id="Dogrulama" name="pwdDogrulama" type="password" class="form-control " placeholder="Yeni Þifre Doðrulama" />',
